...
| ✏ **Настоящий редактор** | перетаскивание блоков, правка узлов линий, перепривязка стрелок, палитра фигур, схемы с нуля, отмена, экспорт |
...
| 🌗 **Светлая и тёмная** | одна кнопка меняет и интерфейс, и сами схемы; что видите, то и экспортируется |
| 🌍 **Два языка** | русский и английский везде — интерфейс, выбор в консоли и подписи на схемах |
| 🎒 **Переносимость** | парсеры лежат в репозитории: клонировал и запустил — или собери один исполняемый файл |

> [!NOTE]
> Утилита следует **ГОСТ 19.701-90**, поэтому по умолчанию всё на русском.
> Ключ `--ui-lang en` (или кнопка **RU/EN** в редакторе) переводит интерфейс,
> выбор в консоли и подписи на схемах на английский.
...
| `--theme {light,dark}` | палитра схем и альбома |
| `--ui-lang {ru,en}` | язык интерфейса и подписей на схемах |
...
- **Блоки** перетаскиваются, ручки меняют размер. Линии прокладывает
  [libavoid](https://github.com/mjwybrow/adaptagrams) — тот же трассировщик,
  что у соединительных линий Inkscape: ортогональные маршруты обходят блоки,
  выходят и входят строго по нормали к стороне, а линия, которую блок лишь
  задел, сохраняет свой ход. Стыки не отрываются, а если вернуть блок на место,
  линии становятся ровно такими, как были.
- **Линии** показывают узлы по щелчку — их можно таскать, добавлять, а крайний
  ...
  строит маршрут с нуля и сам выбирает стороны блоков.
...
- **＋** создаёт пустую схему: алгоритм можно нарисовать и без кода; кнопка ✎
  (или двойной щелчок по названию) переименовывает любую схему.
- **Тема и язык** переключаются в правом верхнем углу и запоминаются между
  сеансами — вместе с галочкой **заголовок**, которая решает, попадёт ли
  подпись в экспортируемый SVG или PNG.
...
- **Телефоны и планшеты**, вплоть до ширины 320 px: список схем выезжает по
  ☰, экспорт и настройки спрятаны под ⋯, а панель свойств поднимается снизу,
  когда вы касаетесь блока (в альбомной ориентации — сбоку). Холст листается
  пальцем по пустому месту, масштаб — щипком; схема открывается вписанной в экран.
...
браузере, причём редактор рассчитан на палец: коснитесь блока — снизу выедет
панель правки, блоки перетаскиваются, масштаб меняется щипком, PNG сохраняется
в галерею.
...
Пока ничего не тронуто, линии остаются такими, какими их нарисовала разметка.
При правке libavoid — собранный в WebAssembly и встроенный прямо в `index.html`
(≈ 0,7 МБ) — перекладывает только затронутые линии. Ни сервер, ни сеть не
нужны: страница работает с `file://` и в Windows, и в Linux, и на Android. Если
браузер не поддержит WebAssembly, редактор перейдёт на простой встроенный
алгоритм.
...
tools/
├── vendor.py             пересборка vendor/: парсеры и трассировщик линий
...
[C++](https://github.com/tree-sitter/tree-sitter-cpp). Линии в редакторе
прокладывает libavoid из [Adaptagrams](https://github.com/mjwybrow/adaptagrams)
в сборке для браузера [libavoid-js](https://github.com/Aksem/libavoid-js).
...
`vendor/web/libavoid` — это libavoid-js под лицензией
[LGPL-2.1-or-later](vendor/web/libavoid/LICENSE), отдельный неизменённый модуль
WebAssembly; скачать его заново — `python tools/vendor.py --web`. Все сторонние
компоненты и тексты их лицензий собраны в
